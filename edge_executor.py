...
class EdgeExecutor(BaseExecutor):
    ...
    def _check_db_schema(self, engine: Engine) -> None:
        ...
        with contextlib.suppress(NoSuchTableError):
            edge_worker_columns = [
                column["name"] for column in inspector.get_columns("edge_worker")
            ]
    ...
    def queue_workload(
        ...
        if not isinstance(workload, workloads.ExecuteTask):
            raise TypeError(
                f"Don't know how to queue workload of type {type(workload).__name__}"
            )
    ...
    def _update_orphaned_jobs(self, session: Session) -> bool:
        ...
        heartbeat_interval: int = conf.getint(
            "scheduler", heartbeat_interval_config_name
        )
