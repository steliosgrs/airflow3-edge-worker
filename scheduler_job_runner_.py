...
class SchedulerJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def _maybe_requeue_stuck_ti(self, *, ti, session):
        ...
        if num_times_stuck < self._num_stuck_queued_retries:
            self.log.info(
                "Task stuck in queued; will try to requeue. task_instance=%s", ti
            )
            session.add(
                Log(
    ...
    def _generate_task_instance_heartbeat_timeout_message_details(
        ...
        if ti.external_executor_id:
            task_instance_heartbeat_timeout_message_details["External Executor Id"] = (
    ...
    def _try_to_load_executor(self, executor_name: str | None) -> BaseExecutor | None:
        ...
        log to the user.
        """
        self.log.info("Executor: %s", executor_name)
        if executor_name is None:
            return self.job.executor

        for e in self.job.executors:
            # BUG FIX: Add more comprehensive matching logic
            # Check against the executor class name as well
            executor_class_name = e.__class__.__name__
            executor_module_path = f"{e.__class__.__module__}.{e.__class__.__name__}"

            # Original matching logic
            if (
                e.name.alias == executor_name
                or e.name.module_path == executor_name
                or
                # NEW: Match against class name (e.g., "EdgeExecutor")
                executor_class_name == executor_name
                or
                # NEW: Match against full module path
                executor_module_path == executor_name
            ):
                return e
        ...
        # DEBUG: Log available executors for troubleshooting
        self.log.debug("Available executors:")
        for e in self.job.executors:
            self.log.debug(
                "  - Class: %s, Name: %s, Alias: %s, Module: %s",
                e.__class__.__name__,
                getattr(e.name, "name", "N/A"),
                getattr(e.name, "alias", "N/A"),
                getattr(e.name, "module_path", "N/A"),
            )
